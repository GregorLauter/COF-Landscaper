#!/usr/bin/env python3
"""End-to-end COF-Landscaper workflow runner from an existing preoptimized layer.
...
DEFAULT_PARAMS_FILE = Path(__file__).with_name("cof-landscaper_from_cif.params.json")


def run_workflow(params: dict[str, object]) -> None:
    """Run the batch computational workflow using JSON parameters."""
    ...
    mace_opt.run(cof_name=cof_name, mode=mode)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run COF-Landscaper from an existing preoptimized layer.",
    )
    parser.add_argument(
        "--params",
        "-p",
